...
void load_ind_direct_long(vm_t *vm, champ_t *champ)
{
	int ind = get_indirect_as_int(vm->ram, (champ->pc + 2) % MEM_SIZE);
	int reg = vm->ram[(champ->pc + 6) % MEM_SIZE];
	int s = get_indirect_as_int(vm->ram, (champ->pc + ind) % MEM_SIZE);
...
void load_two_direct_long(vm_t *vm, champ_t *champ)
{
...
int vm_lldi(vm_t *vm, champ_t *champ)
{
	int type;
	...
	if (FIRST_PARAM_TYPE(type) == DIRECT_TYPE)
		load_two_direct_long(vm, champ);
	else
		load_ind_direct_long(vm, champ);
	return (7);
}
