...
void copi_lchamp(champ_t *champ, champ_t *new, int arg)
{
...
int vm_lfork(vm_t *vm, champ_t *champ)
{
	...
	if (vm == NULL || champ == NULL)
		return (-1);
	arg = get_indirect_as_int(vm->ram, ((champ-> pc + 2) % IDX_MOD) \
% MEM_SIZE);
	copi_lchamp(champ, new_champ, arg);
