...
static int load_indirect_to_register_lld(byte_t *ram, champ_t *champ)
{
	...
	indirect_start = champ->pc + indirect_value;
...
static int load_direct_to_register_lld(byte_t *ram, champ_t *champ)
{
	byte_t *register_start = NULL;
	int direct_start = 0;
	byte_t register_nb = 0;
...
int vm_lld(vm_t *vm, byte_t *instruction_pos, champ_t *champion)
{
	...
	if (FIRST_PARAM_TYPE(types) == DIRECT_TYPE)
		load_direct_to_register_lld(vm->ram, champion);
	else
		load_indirect_to_register_lld(vm->ram, champion);
